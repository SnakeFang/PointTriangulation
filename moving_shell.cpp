...
std::vector<line> triangulate_moving_shell(const std::vector<point*>& points)
{
    ...
    for (point* point_new : points_all)
    {
        ...
        for (point* point_shell : points_shell)
        {
            ...
            if (!std::any_of(lines_shell.begin(), lines_shell.end(), std::bind(&lines_intersect, std::ref(line_new), std::placeholders::_1)))
            {
                lines_shell.push_back(line_new);
                lines_all.push_back(line_new);
